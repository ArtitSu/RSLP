...
__global__ void findLeave_index(double *devicemin_P, double *devicex_B, int basic_number, 
    double *findArray) {
    ...
    if (findArray[0] == 1){
        if ( i < basic_number){
            if(devicemin_P[i] > 0){
                double ratio = devicex_B[i] / devicemin_P[i];
                if ( ratio < min_ratio[0] && ratio >= 0){
                    findArray[1] = ratio;
                    findArray[2] = i;
                    findArray[3] = 0;               
                }
                else if (ratio == min_ratio[0]){
                    repeat[0]++;
                }
            }
        }
...
__global__ void initdeviceA(double *deviceA, double * deviceAineq, double * deviceAeq, int Mineq, int Meq, int N){
    ...
    if ( k < Mineq + Meq){
        if ( k < Mineq){
            for (int i = 0 ; i < N ; i++){
                deviceA[ i * (Mineq + Meq) + k] = deviceAineq[i * (Mineq + Meq) + k];
            }
        }
        else if ( k >= Mineq && k < Mineq + Meq){
            for (int i = 0 ; i < N ; i++){
                deviceA[ i * (Mineq + Meq) + k] = deviceAineq[i * (Mineq + Meq) + k - Mineq];
            }
        }
    }
}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ){
    double *deviceC, *deviceA, *deviceMatrix, *deviceVector, *deviceVectorN, *deviceI, *deviceAeq, *deviceAineq, *devicex_B, *devicemin_P;
    int *deviceIpiv;
    ...
    cudaMalloc(&devicemin_P, sizeof(double) * basic_number );
    cudaMalloc(&devicex_B, sizeof(double) * basic_number );
    ...
    //define c_basic
    double* c_basic = new double[basic_number];
    ...
    //artificial column
    double* art = new double[basic_number];
    ...
    while (phaseI == true){
        ...
        if ( min_c_d >= 0 ){
            ...
            cudaMemcpy(devicemin_P, min_P, sizeof(double) * basic_number, cudaMemcpyHostToDevice);
            cudaMemcpy(devicex_B, x_B, sizeof(double) * basic_number, cudaMemcpyHostToDevice);
            unbound = true;

            bool *ub = new bool[1];
            ub[0] = true;
            double *min_ratio = new double[1];
            min_ratio[0] = 999999999999999999999999999999999.0;
            int *repeat = new int[1];
            repeat[0] = 0;
            int *li = new int[1];
            li[0] = -1;
            leave_index = -1;
            findLeave_index<<<blocksPerGrid, threadsPerBlock>>>(devicemin_P, devicex_B, basic_number, min_ratio, li, repeat, ub);
            unbound = ub[0];
            leave_index = li[0];
            ...
            if (repeat[0] != 0){
                unbound = true;
                fprintf(logfile, "leave_index is repeated\n");
            }
            if (unbound == true){
                break;
            }
    ...
    while(infeasible == false){
        ...
        if ( min_c_d >= 0 ){
            ...
            unbound = ub[0];
            leave_index = li[0];
            fprintf(logfile, "min_ratio = %9.9f\n", min_ratio[0]);
            if (leave_index == -1){
                unbound = true;
                fprintf(logfile, "leave_index = -1\n");
            }
