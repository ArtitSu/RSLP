...
__global__ void initdeviceA(double *deviceA, double * deviceAineq, double * deviceAeq, int Mineq, int Meq, int N){
    ...
    if ( k < Mineq + Meq){
        if ( k < Mineq){
            for (int i = 0 ; i < N ; i++){
                deviceA[ i * (Mineq + Meq) + k] = deviceAineq[i * Mineq  + k];
            }
        }
        else if ( k >= Mineq && k < Mineq + Meq){
            for (int i = 0 ; i < N ; i++){
                deviceA[ i * (Mineq + Meq) + k] = deviceAineq[i *  Meq + k - Mineq];
            }
        }
    }
}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ){
    ...
    while (phaseI == true){
        ...
        for(int i = 0 ; i < non_basic_number+1 ; i++){
            ...
            for(int row = 0 ; row < basic_number ; row++){
                for(int col = 0 ; col < 1 ; col++){
                    fprintf(logfile, "%9.9f\t", P[col*basic_number + row]);
                }
                fprintf(logfile, "\n");
            }
            ...
            for ( int i = 0 ; i < basic_number ; i++){
                if(min_P[i] > 0){
                    ...
                    if ( ratio < m && ratio >= 0){
                        ...
                        leave_index = i;
                        repeat = 0;
                        fprintf(logfile, "min ratio_%d = %9.9f / %9.9f = %9.9f\n", i, x_B[i], min_P[i], ratio);
                    }
                    else if (ratio == m){
                        fprintf(logfile, "ratio_%d = %9.9f, m = %9.9f\n", i, ratio, m);
                        repeat++;
